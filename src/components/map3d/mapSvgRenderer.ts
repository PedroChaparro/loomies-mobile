import { iPosition, iNode, iWay, iMap } from './mapInterfaces';
import { BBOX_SIDE_LENGTH } from './osm';
...
export const mapToSVG = (map: iMap): string => {
  ...
  let svg = '<svg xmlns="http://www.w3.org/2000/svg" width="500" height="500" viewBox="0 0 500 500">';
...
const drawWays = (map: iMap, ways: iWay[], prefix: string, sufix: string): string => {
  let svg = "";
  let nodeMapXCoord = 0
  let nodeMapYCoord = 0
  ...
  ways.forEach((way: iWay) => {
    ...
    way.nodes.forEach((nodeIdentifier: string) => {
      ...
      nodeMapXCoord = Math.round(((node.position.lon - map.origin.lon) / (BBOX_SIDE_LENGTH * 2) ) * MAP_SIZE);
      nodeMapYCoord = MAP_SIZE - Math.round(((node.position.lat - map.origin.lat) / (BBOX_SIDE_LENGTH * 2) ) * MAP_SIZE);
